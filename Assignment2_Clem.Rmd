...
```{r}
#Load Packages
pacman::p_load(tidyverse, rethinking, brms, patchwork)
...
precis( ACI_MR ) #b_MR  0.12 0.10 -0.05  0.28
...
alist(
ACI_scale ~ dnorm( mu , sigma ) ,
mu <- a + b_Apath * Apath_scale,
a ~ dnorm( 0 , 0.2 ) ,
b_Apath ~ dnorm( 0 , 0.5 ) ,
sigma ~ dnorm(1, 0.5)
) , data = d2 )

precis(ACI_Apath) #b_Apath -0.09 0.12 -0.28  0.10
...
precis(ACI_VH) #b_VH  0.14 0.12 -0.05  0.33
...
alist(
ACI_scale ~ dnorm( mu , sigma ) ,
mu <- a + b_VH * VH_scale + b_MR * MR_scale + b_Apath * Apath_scale,
a ~ dnorm( 0 , 0.2 ) ,
b_VH ~ dnorm( 0 , 0.5 ) ,
b_MR ~ dnorm(0, 0.5),
b_Apath ~ dnorm(0, 0.5),
...
precis(ACI_all) #The estimates are still all too uncertain to say anything about.
