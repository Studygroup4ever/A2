...
mean(posteriorAPA$b_APA)
sd(posteriorAPA$b_APA)

ggplot(posteriorAPA) +
  geom_density(aes(b_APA), fill="blue", alpha=0.2) +
  theme_minimal()
...
mean(posteriorVH$b_VH)
sd(posteriorVH$b_VH)

ggplot(posteriorVH) +
  geom_density(aes(b_VH), fill="blue", alpha=0.2) +
  theme_minimal()
...
# Visualizing the priors
prior_mTwo <- prior_samples(model_mTwo)
prior_mTwo %>% glimpse()
...
# Sampling from the posterior
posterior_mTwo <- posterior_samples(model_mTwo)
