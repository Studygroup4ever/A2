...
# Apathy
ModelAPA <- bf(ACI ~ 1 + APA)

prior_defAPA <- c(
  prior(normal(0, 0.5), class = Intercept), # prior set to the overall mean of the data, aka. the same data prior for the 0 + Diagnosis model made earlier,
...
# Plotting the prior and posterior distributions
  ...
  xlab("Altercentric Intrusion (std)")+ggtitle("Posterior Predictive Distributions, ACI ~ 1 + VoiceHearing + Mindreading")
...
# Plotting the prior and posterior distributions
  ...
  xlab("Altercentric Intrusion (std)")+ggtitle("Posterior Predictive Distributions, ACI ~ 1 + Mindreading + VoiceHearing + Apathy")
...
m_ALL_full <- quap(
  alist(
    ACI ~ dnorm(mu , sigma) ,
    mu <- a + b_VH * VH + b_MR * MR + b_Apath * APA,
    a ~ dnorm( 0 , 0.2 ),
    b_VH ~ dnorm(0 , 0.5) ,
    b_MR ~ dnorm(0, 0.5),
    b_Apath ~ dnorm(0, 0.5),
...
ggplot(posterior6) +
  ...
  theme_minimal()+
  ggtitle("ACI ~ 1 + Mindreading + VoiceHearing, All participants")
